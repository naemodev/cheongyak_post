...
// Normalizes every row into the shared internal shape (see src/model.js).
...
async function fetchCategory(endpoint, sinceDate) {
  ...
  while (true) {
    ...
    const url = `${config.applyhome.base}/${endpoint.op}?${params.toString()}`;
